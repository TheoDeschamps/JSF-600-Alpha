...
// Créer un channel
async function createChannel(io, socket, channelName) {
...
// Lister les channels
async function listChannels(io, socket, keyword = '') {
    try {
...
function joinChannel(io, socket, channelName) {
    if (!channelName || channelName.trim() === '') {
        socket.emit('error', 'Channel name cannot be empty');
        return;
    }
...
// Quitter un channel
function quitChannel(io, socket, channelName) {
    const nickname = nicknames.get(socket.id);
    socket.leave(channelName);
    io.to(channelName).emit('user_left', `${nickname} left ${channelName}`);
}
// Supprimer un channel
async function deleteChannel(io, socket, channelName) {
    if (!channelName || channelName.trim() === '') {
        socket.emit('error', 'Channel name cannot be empty');
        return;
    }
